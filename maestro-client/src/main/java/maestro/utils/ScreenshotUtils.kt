...
class ScreenshotUtils {
    companion object {
        ...
        /**
         * Takes a screenshot capping the underlying driver call at [callTimeoutMs] ms.
         * On drivers that ignore the timeout (Android/web today), behavior is identical
         * to [tryTakingScreenshot]. On iOS, the OkHttp call to XCUITest's `/screenshot`
        ...
        fun waitUntilScreenIsStatic(timeoutMs: Long, threshold: Double, driver: Driver): Boolean {
            ...
            // a single slow `takeScreenshot` (e.g. XCUITest during animation) cannot blow past
            // the user-supplied [timeoutMs]. Drivers that don't honor the timeout will still
            // finish in their own time, but the second screenshot is never started once the
            // deadline has passed and the loop exits as soon as the body returns.
